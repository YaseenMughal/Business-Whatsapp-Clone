...
class UserChatView extends StatefulWidget {
  ...
  const UserChatView({super.key, required this.name, required this.image});
...
class _UserChatViewState extends State<UserChatView> {
  ...
  List<MessageModel> messagesList = [
    // Adding data into model for today's message
    MessageModel(timeStamp: DateTime.now().microsecondsSinceEpoch, message: "Allah ka shukar h sale achi h", isMe: false),

    MessageModel(timeStamp: DateTime.now().microsecondsSinceEpoch, message: "Kam kesa chl rha h ", isMe: true),
    MessageModel(timeStamp: DateTime.now().microsecondsSinceEpoch, message: "Allah ka Karam, tm sunao!", isMe: false),
    MessageModel(timeStamp: DateTime.now().microsecondsSinceEpoch, message: "bhai ki haal h, tusi thc hoo.", isMe: true),
    MessageModel(timeStamp: DateTime.now().microsecondsSinceEpoch, message: "Walikum Assalam", isMe: false),
    MessageModel(timeStamp: DateTime.now().microsecondsSinceEpoch, message: "Assalamualikum! ", isMe: true),
    ...
    MessageModel(timeStamp: DateTime(2023, 02, 08, 15, 20).microsecondsSinceEpoch, message: "Feb, 08 Message", isMe: true),
    MessageModel(timeStamp: DateTime(2023, 02, 08, 15, 20).microsecondsSinceEpoch, message: "Feb, 08 Message", isMe: false),
    MessageModel(timeStamp: DateTime(2023, 02, 08, 15, 20).microsecondsSinceEpoch, message: "Feb, 08 Message", isMe: true),
    MessageModel(timeStamp: DateTime(2023, 01, 20, 15, 20).microsecondsSinceEpoch, message: "Jan, 20 Message", isMe: true),
    MessageModel(timeStamp: DateTime(2023, 01, 20, 15, 20).microsecondsSinceEpoch, message: "Jan, 20 Message", isMe: false),
    MessageModel(timeStamp: DateTime(2023, 01, 20, 15, 20).microsecondsSinceEpoch, message: "Jan, 20 Message", isMe: true),
    MessageModel(timeStamp: DateTime(2023, 01, 20, 15, 20).microsecondsSinceEpoch, message: "Jan, 20 Message", isMe: false),
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        //                 minScale: 1.0, maxScale: 6.0, child: ImageBuilderWidget(height: MediaQuery.of(context).size.height * .3, width: double.infinity, image: widget.image)),
        //           )),
        //   child: CircleAvatar(
        //     radius: 30,
        //     child: ImageBuilderWidget(height: 60, width: 50, image: widget.image),
        //   ),
        // ),
        title: Row(
          children: [
                              ...
                              Center(
                                  child: Container(
                                      ...
                                      child: Padding(
                                        ...
                                        child: Text(
                                          newDate,
                                          style: const TextStyle(fontSize: 14.0, fontWeight: FontWeight.w400, color: AppColor.whiteColor, fontFamily: "Kanit"),
                                        ),
                                      ))),
                            Padding(
                                ...
                                child: Container(
                                  ...
                                  child: Stack(
                                    children: [
                                      Padding(
                                        padding:
                                            messagesList[index].isMe ? const EdgeInsets.only(left: 4, right: 4, bottom: 10) : const EdgeInsets.only(left: 4, right: 4, bottom: 10),
                                        child: Text(
                                          ...
                                          style: Theme.of(context)
                                              .textTheme
                                              .headlineSmall!
                                              .copyWith(fontSize: 15, color: messagesList[index].isMe ? const Color(0xff705982) : const Color(0xff677D81)),
                                        ),
                                      ),
                                      Positioned(
                                          bottom: 0,
                                          right: 0,
                                          child: Text(
                                            messageTime(messagesList[index].timeStamp.toString()).toString(),
                                            textAlign: TextAlign.left,
                      ...
                      Expanded(
                        child: TextFormField(
                          ...
                          decoration: const InputDecoration(
                              hintText: 'Enter Message...', fillColor: Colors.red, contentPadding: EdgeInsets.symmetric(horizontal: 15), border: OutlineInputBorder()),
                        ),
                      ),
                      const SizedBox(
                        width: 10,
